...
- **النموذج يُعرَّف بعد المتجهات لا قبلها**، وإلا بدأ Meilisearch يضمّن المدوّنة كلها على خادم البحث. لذلك `pnpm index` لا يلمس `embedders` أبدًا؛ الملف `meilisearch-embedder.json` يملكه `pnpm embed` ويطبّقه في آخر خطواته.
